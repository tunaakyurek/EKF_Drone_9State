%% main_improved.m - Improved EKF Drone Simulation with Enhanced Turn Performance
clear; clc; close all;
...
% Store turn detection for analysis
turn_detected_hist = false(1,N);
noise_scale_hist = ones(1,N);

%% PID controller gains (tuned for smooth, reasonable speed)
Kp = [0.015; 0.015; 0.022];   % Position proportional gains [x;y;z]
Ki = [0.0004; 0.0004; 0.0006]; % Integral gains
Kd = [0.035; 0.035; 0.04];   % Derivative gains
Kp_yaw = 0.2; Ki_yaw = 0.002; Kd_yaw = 0.04;
...
Kp_att = [1.2; 1.2; 0.8]; % [roll; pitch; yaw]
Kd_att = [0.4; 0.4; 0.2];
...
for k = 1:N
    ...
    vel_damping = -0.2 * vel; % Damping gain
    ...
    % Simulate true drone dynamics with improved model
    ...
    % EKF predict (IMU rate) - Use adaptive EKF
    if mod(k-1, round(params.Ts.IMU/dt)) == 0
        [x_est, P] = ekf_adaptive(x_est, P, imu_meas, [], params, params.Ts.IMU, 'IMU');
    end
    
    % EKF update (GPS)
    if mod(k-1, round(params.Ts.GPS/dt)) == 0
        [x_est, P] = ekf_adaptive(x_est, P, imu_meas, sensors.gps, params, params.Ts.GPS, 'GPS');
        gps_hist(:,k) = sensors.gps;
    end
    
    % EKF update (Baro)
    if mod(k-1, round(params.Ts.Baro/dt)) == 0
        [x_est, P] = ekf_adaptive(x_est, P, imu_meas, sensors.baro, params, params.Ts.Baro, 'Baro');
    end
    
    % EKF update (Mag)
    if mod(k-1, round(params.Ts.Mag/dt)) == 0
        [x_est, P] = ekf_adaptive(x_est, P, imu_meas, sensors.mag, params, params.Ts.Mag, 'Mag');
    end
