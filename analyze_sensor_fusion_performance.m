...
subplot(4,3,10);
analyze_sensor_contributions(t, gps_hist, baro_hist, mag_hist);

%% NOISE CHARACTERISTICS
subplot(4,3,11);
analyze_noise_characteristics(t, imu_hist, gps_hist);
...
for i = 1:length(t)
    if baro_available(i)
        true_altitude = -x_true_hist(3,i); % Convert NED down to altitude up
        measured_altitude = -baro_hist(i);  % Convert NED down to altitude up
...
for i = 1:length(t)
    if gps_available(i) && baro_available(i)
        gps_altitude = -gps_hist(3,i);     % Convert NED down to altitude up
        baro_altitude = -baro_hist(i);     % Convert NED down to altitude up
